...
typedef NS_ENUM(NSInteger, TBStoreOrderFilter) {
    TBStoreOrderFilter_All     = 0,
    TBStoreOrderFilter_Unpaid  = 1,
    TBStoreOrderFilter_Paid
};
 ...
 * Before calling this method, you must first sign in your current app user to SDK by calling signinSDKUser:fromApp:completionBlock: Without doing so will cause the viewcontroller not able to find any matching orders.
 ...
 @param filter TBStoreOrderListFilter
 @return orderListVC
 */
- (UIViewController *)orderListViewController:(TBStoreOrderFilter)filter;
